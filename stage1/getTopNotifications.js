...
async function loadToken() {
  // Priority: explicit --token, env ACCESS_TOKEN, stage1 .env, backend .env
  ...
  const envFiles = [
    ...
    path.resolve(__dirname, "..", "notification_app_be", ".env"),
  ];
...
async function fetchNotifications(token) {
  ...
  Log("backend", "info", "service", `Fetching notifications from ${url}`);
...
async function main() {
  ...
  if (!token) {
    ...
    Log("backend", "error", "service", "Missing access token for notifications API");
    process.exit(1);
  }
  ...
  try {
    ...
    Log("backend", "info", "service", `Top ${topN} notifications calculated`);
    ...
    Log("backend", "error", "service", `Failed to fetch or process notifications: ${err.message}`);
